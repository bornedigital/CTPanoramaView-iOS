...
@objc public protocol CTPanoramaTapDelegate {
    func panoramaTapped(on: CGPoint)
}
...
@objc public class CTPanoramaView: UIView {
    ...
    @objc public var tapDelegate: CTPanoramaTapDelegate?
    ...
    private func switchControlMethod(to method: CTPanoramaControlMethod) {
        ...
        if method == .touch {
            ...
            let tapRec = UITapGestureRecognizer(target: self, action: #selector(handleTap(tapRec:)))
    ...
    @objc func handleTap(tapRec: UITapGestureRecognizer) {
        let location = tapRec.location(in: sceneView)
        let hitResults = sceneView.hitTest(location, options: nil)
        if hitResults.count > 0 {
            let result = hitResults[0]
            let textureCoordinates = result.textureCoordinates(withMappingChannel: 0)
            let normalTextureCoordinates = CGPoint(x: 1.0 - textureCoordinates.x, y: textureCoordinates.y)
            self.tapDelegate?.panoramaTapped(on: normalTextureCoordinates)
        }
    }
